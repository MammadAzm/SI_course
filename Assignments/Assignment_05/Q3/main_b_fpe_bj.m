...
bj_FPEModel_guassian = bj(data_guassian, [na nb nc nd nk]);
FPE_y_hat_guassian = lsim(bj_FPEModel_guassian, u1_val, t_val);
...
for degree=1:100
    ...
    try
        ...
        bj_y_hat_prbs = lsim(sys, u2, t);
    catch
        break
    end

    error = y2 - bj_y_hat_prbs;
    S_hat = 0;
    for i=1:length(error)
        S_hat = S_hat + error(i)^2;
    end
...
bj_FPEModel_prbs = bj(data_prbs, [na nb nc nd nk]);
FPE_y_hat_prbs = lsim(bj_FPEModel_prbs, u2_val, t_val);
